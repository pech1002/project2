...
class  optimization(object):
    ...
    def SBR1_G(self,Hk,sk,yk):
        

        Qk = np.linalg.inv(Hk)
        yQs = yk - Qk*sk 
        Qk = Qk +(yQs*yQs.T)/(yQs.T*sk) 
        Hk = np.linalg.inv(Qk)
                        
        return Hk  
        
    def SBR1_B(self,Hk,sk,yk):  #Simple Broyden rank-1 update of H = G−1 ("bad Broyden")
    
        Hk = Hk + ((sk - Hk*yk)/(yk.T*yk))*yk.T
        
        return Hk
      
    def SR1(self,Hk,sk,yk): # Hk = np.eye(2)  P51/52  Symmetric Broyden update
    ...
    def DFP(self,Hk,sk,yk):
        Hk = Hk - (Hk * yk * yk.T * Hk) / (yk.T * Hk * yk) +\
                    (sk * sk.T) / (sk.T* yk) # DFP method
        return Hk
    
    def BFGS(self,Hk,sk,yk):
        
        yHy = (yk.T * Hk * yk) / (sk.T * yk)
        yHy = float(yHy)
        Hk = Hk + (1 + yHy) * ((sk * sk.T)/(sk.T * yk)) \
    ...
    def Quasi_Newton(self):
        ...
        alfa_L = 0.0
        #Hk = np.eye(2) # SR1
        Hk = np.linalg.inv(hess(x0)) # DFP and BFGS SBR1_B
        Hk = np.matrix(Hk)
        while(k < k_max):           
            gk = gfun(x0)
            s_k = -gk*Hk
            ...
            # Transpose (1,n) to (n,1)
            sk = sk.T
            yk = yk.T
            
            if yk.T * sk> 0:                  
                #Hk = self.SR1(Hk,sk,yk)
                #Hk = self.SBR1_B(Hk,sk,yk)
                #Hk = self.SBR1_G(Hk,sk,yk)
